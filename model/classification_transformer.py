...
# built on top of ablateself since that performed better
class DifferentHiddenStateModel(nn.Module):
    def __init__(self, mconfigs):
        super().__init__()
        #get which hidden state to use
        self.hidden_state_n = mconfigs['hidden_state_n']
    ...
    def forward(self, x):
        ...
        # pass into pretrained encoders to get embeddings
        prot_embedding = self.prot_encoder(prot, attention_mask = prot_2dmask, output_hidden_states=True)['hidden_states'][self.hidden_state_n] #torch.FloatTensor of shape (batch_size, prot_seq_len, prot_demb=320)
        dna_embedding = self.dna_encoder(dna, attention_mask = dna_2dmask, output_hidden_states=True)['hidden_states'][-1] #(batch_size, dna_seq_len, dna_demb=2560)
        
        # later, get multiple hidden states, concat
